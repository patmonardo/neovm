...
class PropertyMappingImpl implements PropertyMapping {
  ...
  setNonDefaultAggregation(aggregation: Aggregation): PropertyMapping {
    if (aggregation === Aggregation.DEFAULT || this._aggregation !== Aggregation.DEFAULT) {
      return this;
    }
  ...
  private validateProperties(): void {
    if (this.neoPropertyKey() === ElementProjection.PROJECT_ALL && this._aggregation !== Aggregation.COUNT) {
      throw new Error("A '*' property key can only be used in combination with count aggregation.");
    }
...
export namespace PropertyMapping {
   ...
   */
  export function fromObject(propertyKey: string, stringOrMap: any): PropertyMapping {
    if (typeof stringOrMap === 'string') {
      const neoPropertyKey = stringOrMap;
      return fromObject(
        propertyKey,
        { [PROPERTY_KEY]: neoPropertyKey }
      );
      ...
      if (!propertyMap.has(aggregationKey)) {
        ...
        if (typeof aggregationValue === 'string') {
          ...
          throw new Error(
            `Expected the value of '${RelationshipProjection.AGGREGATION_KEY}' to be of type String, but was '${typeof aggregationValue}'`
          );
        }
      }
   ...
   * @returns A new PropertyMapping
   */
  export function of(propertyKey: string, options?: PropertyMappingOptions | string): PropertyMapping {
