 ...
 */
export interface GraphStoreToDatabaseExporterConfig extends GraphStoreExporterBaseConfig, JobIdConfig {
  /**
...
export namespace GraphStoreToDatabaseExporterConfig {
   ...
   */
  export function of(config: CypherMapWrapper): GraphStoreToDatabaseExporterConfig {
    const normalizedConfig = config.getString(DB_NAME_KEY)
      .map(dbName => {
 ...
 */
export class GraphStoreToDatabaseExporterConfigImpl implements GraphStoreToDatabaseExporterConfig {
   ...
   */
  databaseName(): string {
    return this.config.requireString(GraphStoreToDatabaseExporterConfig.DB_NAME_KEY);
  }
   ...
   */
  databaseFormat(): string {
    return this.config.getString(GraphStoreToDatabaseExporterConfig.DB_FORMAT_KEY)
  ...
  defaultRelationshipType(): RelationshipType {
    return this.config.getRelationshipType(GraphStoreExporterBaseConfig.DEFAULT_RELATIONSHIP_TYPE_KEY)
      .orElse(RelationshipType.of("__ALL__"));
  }

  batchSize(): number {
    return this.config.getNumber(GraphStoreExporterBaseConfig.BATCH_SIZE_KEY)
      .orElse(GraphStoreExporterBaseConfig.DEFAULT_BATCH_SIZE);
  }

  writeConcurrency(): Concurrency {
    return this.config.getConcurrency(GraphStoreExporterBaseConfig.WRITE_CONCURRENCY_KEY)
   ...
   */
  toString(): string {
    return `GraphStoreToDatabaseExporterConfig{` +
