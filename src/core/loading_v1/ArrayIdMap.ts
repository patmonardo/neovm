...
import { OptionalLong } from '../utils/OptionalLong';
...
import { LabeledIdMap } from './LabeledIdMap';
...
export class ArrayIdMap extends LabeledIdMap {
  private static readonly ESTIMATION: MemoryEstimation = MemoryEstimations
    .builder(ArrayIdMap) // In TS, passing the class itself is for informational purposes
    ...
    .perGraphDimension(
      ...
      (dimensions, concurrency) =>
        MemoryRange.of(BigInt(dimensions.estimationNodeLabelCount()) * Estimate.sizeOfBitset(dimensions.nodeCount()))
    )
    .build();
  ...
  public override withFilteredLabels(
    ...
    const unionBitSet = this.labelInformation.unionBitSet(nodeLabels, this.nodeCount());

    if (unionBitSet.isEmpty()) { // Added check for empty result after union
        return Optional.empty();
    }
