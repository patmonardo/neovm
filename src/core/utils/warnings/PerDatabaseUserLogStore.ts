...
import { LogStore, LogStoreEntry } from './LogStore'; // Assuming LogStoreEntry is [Task, string[]]
...
export class PerDatabaseUserLogStore implements UserLogStore {
  ...
  public addUserLogMessage(username: string, task: Task, message: string): void {
  ...
  public query(username: string): UserLogEntry[] {
    ...
    return logStore.stream().flatMap(
      (taskWithMessages: LogStoreEntry) => PerDatabaseUserLogStore.taskWithMessagesToUserLogEntryArray(taskWithMessages)
    );
  }
   ...
   */
  private static taskWithMessagesToUserLogEntryArray(taskWithMessages: LogStoreEntry): UserLogEntry[] {
