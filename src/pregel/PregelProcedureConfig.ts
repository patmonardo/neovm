 ...
 */
export interface PregelProcedureConfig extends 
    PregelConfig,
    WritePropertyConfig,
 ...
 */
export abstract class BasePregelProcedureConfig implements PregelProcedureConfig {
    ...
    validateGraphIsSuitableForWrite(
        ...
        if (!graphStore.capabilities().canWriteToLocalDatabase() && 
            ...
            throw new Error("The provided graph does not support `write` execution mode.");
        }
    }
}
 ...
 * Required interfaces
 */
export interface WritePropertyConfig {
    writeProperty(): string;
}

export interface MutateNodePropertyConfig {
    mutateProperty(): string;
}
 ...
 */
export function createPregelProcedureConfig(userInput: CypherMapWrapper): PregelProcedureConfig {
...
class PregelProcedureConfigImpl implements PregelProcedureConfig {
    ...
    concurrency(): number { return 4; }
    maxIterations(): number { return 10; }
    relationshipWeightProperty(): string | undefined { return undefined; }
    isAsynchronous(): boolean { return false; }
    partitioning(): any { return 'RANGE'; } // Assuming Partitioning enum from previous file
    useForkJoin(): boolean { return this.partitioning() === 'AUTO'; }
    trackSender(): boolean { return false; }
}