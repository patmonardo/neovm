import { NodeProjections } from "../NodeProjections"; // Placeholder
import { Orientation } from "../Orientation"; // Placeholder
import { RelationshipProjection } from "../RelationshipProjection"; // Placeholder
import { RelationshipProjections } from "../RelationshipProjections"; // Placeholder
import { RelationshipType } from "../RelationshipType"; // Placeholder
...
import { GraphStoreBuilder } from "../core/loading/GraphStoreBuilder"; // Placeholder
import { Nodes } from "../core/loading/Nodes"; // Placeholder
import { RelationshipImportResult } from "../core/loading/RelationshipImportResult"; // Placeholder
import { NodePropertiesFromStoreBuilder } from "../core/loading/nodeproperties/NodePropertiesFromStoreBuilder"; // Placeholder
import { ProgressTracker } from "../core/utils/progress/tasks/ProgressTracker"; // Placeholder
import { Estimate } from "../mem/Estimate"; // Placeholder
...
import { MemoryUsage } from "../mem/MemoryUsage"; // Placeholder
import { formatWithLocale } from "../utils/StringFormatting"; // Placeholder
...
> extends GraphStoreFactory<CSRGraphStore, CONFIG> {
  ...
  public static getMemoryEstimation(
    ...
    relationshipProjections
      ...
      .forEach((relationshipProjection, relationshipType) => {
        ...
        if (isLoading) {
          builder.max([ // Max of a list of estimations
  ...
  private static addRelationshipEstimationsDuringLoading(
    ...
    estimationBuilder.perNode(
      formatWithLocale("offsets for '%s'%s", relationshipType.name, indexSuffix),
      HugeLongArray.memoryEstimation // Pass the static method itself
    );
    estimationBuilder.perNode(
      formatWithLocale("degrees for '%s'%s", relationshipType.name, indexSuffix),
  ...
  private static relationshipEstimationAfterLoading(
    ...
    const afterLoadingEstimation = MemoryEstimations.builder(
      "size after loading"
    );
    ...
    relationshipProjection
      ...
      .forEach((resolvedPropertyMapping: ResolvedPropertyMapping) => {
        afterLoadingEstimation.add(
          ...
          AdjacencyListBehavior.adjacencyPropertiesEstimation( // Assuming this estimates a single property
            relationshipType,
            undirected
