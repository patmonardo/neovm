...
import { NodePropertyValues, UnsupportedOperationError } from '../abstract/NodePropertyValues'; // Import helper and error

export class DoubleNodePropertyValuesImpl implements DoubleNodePropertyValues {
  ...
  // --- Methods specific to DoubleNodePropertyValues or overriding defaults ---
  ...
  longValue(nodeId: number): number {
    ...
    if (doubleVal === this.defaultValue && this.defaultValue === DefaultValue.DOUBLE_DEFAULT_FALLBACK) {
        return DefaultValue.LONG_DEFAULT_FALLBACK;
    }
    throw new UnsupportedOperationError(`Cannot coerce double value ${doubleVal} at nodeId ${nodeId} to long.`);
  }
