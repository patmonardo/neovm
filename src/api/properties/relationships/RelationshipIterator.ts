...
export interface RelationshipIterator extends RelationshipPredicate {
   ...
   */
  forEachRelationship(nodeId: number, fallbackValue: number, consumer: RelationshipWithPropertyConsumer): void;
   ...
   * @param consumer Relationship consumer function that receives property values
   */
  forEachInverseRelationship(nodeId: number, fallbackValue: number, consumer: RelationshipWithPropertyConsumer): void;
...
export namespace RelationshipIterator {
  ...
  export function empty(): RelationshipIterator {
    return {
      ...
      forEachRelationship(nodeId: number, consumerOrFallback: RelationshipConsumer | number, maybeConsumer?: RelationshipWithPropertyConsumer): void {
        // No-op - no relationships to iterate
      },

      forEachInverseRelationship(nodeId: number, consumerOrFallback: RelationshipConsumer | number, maybeConsumer?: RelationshipWithPropertyConsumer): void {
        // No-op - no relationships to iterate
      },

      iterateRelationships(_nodeId: number, _fallbackValue: number): Iterable<RelationshipCursor> {
        return {
          *[Symbol.iterator]() {
