...
/**
 * A thread-safe version of HugeByteArray that provides atomic operations.
 * Uses SharedArrayBuffer and Atomics API for thread safety.
 */
export abstract class HugeAtomicByteArray implements HugeCursorSupport<Uint8Array> {
  /**
   * Creates a new array of the given size.
   *
   * @param size the length of the new array, the highest supported index is `size - 1`
   * @param pageCreator factory for creating byte array pages
   * @returns new array
   */
  public static of(size: number, pageCreator: PageCreator.BytePageCreator): HugeAtomicByteArray {
   ...
   */
  public abstract update(index: number, updateFunction: ValueTransformers.ByteToByteFunction): void;
   ...
   */
  public abstract newCursor(): HugeCursor<Uint8Array>;

  /**
   * Initialize a cursor for this array.
   */
  public abstract initCursor(cursor: HugeCursor<Uint8Array>): HugeCursor<Uint8Array>;

  /**
   * Initialize a cursor for a range in this array.
   */
  public abstract initCursor(cursor: HugeCursor<Uint8Array>, start: number, end: number): HugeCursor<Uint8Array>;
}

/**
 * Factory for creating HugeAtomicByteArray instances
 */
export class HugeAtomicByteArrayFactory {
  ...
  public static memoryEstimation(size: number): number {
    ...
    if (size <= HugeArrays.MAX_ARRAY_LENGTH) {
      return Estimate.sizeOfInstance(16) + Estimate.sizeOfSharedUint8Array(size);
    }
    ...
    const pageBytes = Estimate.sizeOfSharedUint8Array(HugeArrays.PAGE_SIZE);
    memoryUsed += (numPages - 1) * pageBytes;
    
    const lastPageSize = HugeArrays.exclusiveIndexOfPage(size);
    memoryUsed += Estimate.sizeOfSharedUint8Array(lastPageSize);
 ...
 */
export class SingleHugeAtomicByteArray implements HugeAtomicByteArray {
  private readonly size: number;
  private buffer: SharedArrayBuffer | null;
  private page: Uint8Array | null;

  constructor(size: number, pageCreator: PageCreator.BytePageCreator) {
    this.size = size;
    this.buffer = new SharedArrayBuffer(size);
    this.page = new Uint8Array(this.buffer);
  }
  ...
  public get(index: number): number {
    ...
    return Atomics.load(this.page as any, index);
  }
  ...
  public getAndAdd(index: number, delta: number): number {
    ...
    return Atomics.add(this.page as any, index, delta & 0xFF);
  }
  ...
  public getAndReplace(index: number, value: number): number {
    ...
    return Atomics.exchange(this.page as any, index, value & 0xFF);
  }
  ...
  public set(index: number, value: number): void {
    ...
    Atomics.store(this.page as any, index, value & 0xFF);
  }
  ...
  public compareAndSet(index: number, expect: number, update: number): boolean {
    ...
    return Atomics.compareExchange(this.page as any, index, expect & 0xFF, update & 0xFF) === (expect & 0xFF);
  }
  ...
  public compareAndExchange(index: number, expect: number, update: number): number {
    ...
    return Atomics.compareExchange(this.page as any, index, expect & 0xFF, update & 0xFF);
  }

  public update(index: number, updateFunction: ValueTransformers.ByteToByteFunction): void {
    if (index >= this.size || !this.page) {
      throw new Error(`Index ${index} out of bounds for array of size ${this.size}`);
    }
    ...
    do {
      if (retries++ > MAX_RETRIES) {
        ...
        newValue = updateFunction.applyAsByte(oldValue);
  ...
  public sizeOf(): number {
    return Estimate.sizeOfSharedUint8Array(this.size);
  }
  ...
  public setAll(value: number): void {
    ...
    this.page.fill(byteValue);
  }

  public release(): number {
    if (this.page) {
      const freed = Estimate.sizeOfSharedUint8Array(this.size);
      this.page = null;
      this.buffer = null;
      return freed;
    }
  ...
  public initCursor(cursor: HugeCursor<Uint8Array>, start?: number, end?: number): HugeCursor<Uint8Array> {
    if (!this.page) {
      throw new Error("Array has been released");
    }
 ...
 */
export class PagedHugeAtomicByteArray implements HugeAtomicByteArray {
  private readonly size: number;
  private readonly pageCreator: PageCreator.BytePageCreator;
  private pages: SharedArrayBuffer[] | null;
  private typedPages: Uint8Array[] | null;
  ...
  constructor(size: number, pageCreator: PageCreator.BytePageCreator) {
    ...
    // Memory tracking
    let memoryUsed = Estimate.sizeOfObjectArray(numPages);
    const fullPageMemory = Estimate.sizeOfSharedUint8Array(HugeArrays.PAGE_SIZE);
    
    // Create full pages
    for (let i = 0; i < numPages - 1; i++) {
      const buffer = new SharedArrayBuffer(HugeArrays.PAGE_SIZE);
      this.pages[i] = buffer;
      this.typedPages[i] = new Uint8Array(buffer);
      memoryUsed += fullPageMemory;
    }
    
    // Create last page (might be smaller)
    const lastPageSize = HugeArrays.exclusiveIndexOfPage(size);
    const lastBuffer = new SharedArrayBuffer(lastPageSize);
    this.pages[numPages - 1] = lastBuffer;
    this.typedPages[numPages - 1] = new Uint8Array(lastBuffer);
    memoryUsed += Estimate.sizeOfSharedUint8Array(lastPageSize);
  ...
  public get(index: number): number {
    ...
    return Atomics.load(this.typedPages[pageIndex] as any, indexInPage);
  }
  ...
  public getAndAdd(index: number, delta: number): number {
    ...
    return Atomics.add(this.typedPages[pageIndex] as any, indexInPage, delta & 0xFF);
  }
  ...
  public getAndReplace(index: number, value: number): number {
    ...
    return Atomics.exchange(this.typedPages[pageIndex] as any, indexInPage, value & 0xFF);
  }
  ...
  public set(index: number, value: number): void {
    ...
    Atomics.store(this.typedPages[pageIndex] as any, indexInPage, value & 0xFF);
  }
  ...
  public newCursor(): HugeCursor<Uint8Array> {
    ...
    return new HugeCursor.PagedCursor<Uint8Array>(this.size, this.typedPages);
  }
